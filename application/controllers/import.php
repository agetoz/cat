...
class Import extends CI_Controller {
	...
	public function siswa() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($j = $idx_baris_mulai; $j <= $idx_baris_selesai; $j++) {
                ...
                $nama = $_sheet->getCell("C".$j)->getCalculatedValue();
                $kelas = $_sheet->getCell("D".$j)->getCalculatedValue();

                if ($nim != "" || $nama != "") {
                    $data[] = "('".$nim."', '".$password."', '".$nama."', '".$kelas."')"; 
                }
            }

            $strq = "INSERT INTO m_siswa (nim, password, nama, jurusan) VALUES ";
	...
	public function guru() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($j = $idx_baris_mulai; $j <= $idx_baris_selesai; $j++) {
                ...
                if ($nip != "" || $nama != "") {
                    $data[] = "('".$nip."', '".$password."', '".$nama."')"; 
                }
            }

            $strq = "INSERT INTO m_guru (nip, password, nama) VALUES ";
    ...
    public function soal() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            $tmp    = $target_file.$_FILES['import_excel']['name'];
            //Baca dari tmp folder jadi file ga perlu jadi sampah di server :-p
            
            $this->load->library('excel');//Load library excelnya
            $read   = PHPExcel_IOFactory::createReaderForFile($tmp);
            $read->setReadDataOnly(false);
            $excel  = $read->load($tmp);
