$(document).ready(function() {
	...
	if (url == "m_siswa") {
	...
	} else if (url == "h_ujian_admin") {
		if (url2 == "det") {
			pagination("datatabel", base_url+"/adm/h_ujian_admin/data_det/"+url3, []);
		} else {
			pagination("datatabel", base_url+"/adm/h_ujian_admin/data", []);	
		}
	} else if (url == "h_ujian_guru") {
		if (url2 == "det") {
			pagination("datatabel", base_url+"/adm/h_ujian_guru/data_det/"+url3, []);
		} else {
			pagination("datatabel", base_url+"/adm/h_ujian_guru/data", []);	
		}
	} else if (url == "m_ujian") {
		if (url2 == "det") {
		...
		pagination("datatabel", base_url+"/adm/v_token/data", []);
...
function refresh_token_admin(id) {
	$.ajax({
		type: "GET",
		url: base_url+"/adm/v_token/refresh_token_admin/"+id,
		success: function(response) {
			if (response.status == "ok") {
				pagination("datatabel", base_url+"/adm/v_token/data", []);	
			} else {
...
function m_siswa_u(id) {
	if (confirm('Anda yakin..? Username dan Password otomatis adalah Password pertama dibuat ..!')) {
		$.ajax({
			type: "GET",
...
function m_guru_matkul(id) {
	$.ajax({
		...
		success: function(data) {
			if (data.status == "ok") {
				var jml_data	= Object.keys(data.data).length;
				var hate 	= '<div class="modal fade" id="m_guru_matkul" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting Mapel</h4></div><div class="modal-body"><form name="f_guru_matkul" id="f_guru_matkul" method="post" onsubmit="return m_guru_matkul_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="'+id+'"><div id="konfirmasi"></div>';
